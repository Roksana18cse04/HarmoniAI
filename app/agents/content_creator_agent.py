...
def analyze_instruction_with_llm(platform, model, instruction):
    ...
    price = price_calculate(platform, model, instruction, response)
...
def generate_content_from_instruction(platform, model, instruction):
    ...
    analysis,A_price = analyze_instruction_with_llm(platform, model, instruction)
    # print("analyze------------", analysis)
    price = price_calculate(platform, model, instruction, text_response['content'])
    total_input_token = A_price['input_token']+price['input_token']
    total_output_token = A_price['output_token']+price['output_token']
    total_price = A_price['price']+price['price']
    return {
